...
export async function action({ request }: ActionArgs) {
  ...
  await Nodes.PostNewNode(current.network, current.fromBlockNumber);
  ...
  return (
    <Form method="post">
      ...
      >
        ...
        <VStack w={["full", "full", "58%"]} alignItems={"start"}>
          ...
          <Text fontWeight={"normal"} fontSize={"14px"} color={"gray.500"}>
            Remember you can't change information about the node afterwards, so if you want to make changes, you'll need
            to delete it first and then create a new one.
          </Text>
        </VStack>
      </Flex>
      ...
      <HStack w={"full"} justifyContent={"start"} pt={"12px"} spacing={"10px"}>
        ...
        <Link to={"/nodes/create/blocknumber"}>
          <Button disabled={fetchLoading} size={"sm"} colorScheme={"pink"} variant={"outline"}>
            BACK
          </Button>
        </Link>
        <Button
